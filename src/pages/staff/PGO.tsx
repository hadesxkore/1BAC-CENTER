...
export default function PGO() {
  ...
  const [reports, setReports] = useState<Action[]>([])
  ...
  // Real-time listener for PGO reports
  useEffect(() => {
    setIsLoading(true)
    const q = query(collection(db, 'pgo_reports'), orderBy('createdAt', 'desc'))
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error('Error fetching PGO reports:', error)
      toast.error('Failed to load PGO reports')
      setIsLoading(false)
    })
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DeleteConcernDialog concernId={row.original.id} concernTitle={row.original.reportTitle} collectionName="pgo_reports" />
            </DropdownMenuContent>
          </DropdownMenu>
        )
      },
  ...
  }, [reports, globalFilter, statusFilter, municipalityFilter, categoryFilter, reportTitleFilter, dateFrom, dateTo, advancedSearch])

  // Unique report titles for filter dropdown
  const uniqueReportTitles = useMemo(() => {
    const titles = [...new Set(reports.map((c) => c.reportTitle).filter(Boolean))]
    return titles.sort()
  }, [reports])
  ...
  const generateFormalPDF = async (data: Action[]) => {
    ...
    const addFooter = () => {
      ...
      doc.text(
        `1BAC PGO Reports - Official Report | Page ${pageNum}`,
        margin,
        pageHeight - 8
      )
      doc.text(
    ...
    const addHeader = () => {
      ...
      doc.text('PGO Reports', margin + 5, y + 6)
      doc.setFontSize(9)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(220, 225, 240)
      doc.text('Detailed report Report', margin + 75, y + 6)
      y += 13
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(255, 255, 255)
      doc.text(`report REPORT #${i + 1}`, margin + 5, y + 5)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0 flex-1">
          <h2 className="text-3xl font-heading font-bold">PGO Reports</h2>
          <p className="text-muted-foreground mt-1">
            Monitor and manage all action items and reports
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>PGO Reports</CardTitle>
                <CardDescription>
                  View and manage all reported concerns
                </CardDescription>
              </div>
              <AddConcernDialog collectionName="pgo_reports" />
            </div>
          </CardHeader>
          <CardContent>
            {/* Filters and Search */}
                ...
                <CoordinatesDialog
                  ...
                  onSave={async (coordText) => {
                    ...
                    if (coordText !== undefined) {
                      try {
                        await setDoc(doc(db, 'pgo_reports', rowId), { coordinates: coordText }, { merge: true })
                        setReports((prev) =>
                          prev.map((c) =>
