...
export const generatePNPSummaryPDF = async (
  concerns: Action[],
  dateRange: string
) => {
  ...
  const categoryLabel = 'PNP'
  const categoryColor: [number, number, number] = [220, 38, 38] // Red for PNP
  ...
  doc.setTextColor(255, 255, 255)
  doc.text('PNP REPORTS', pageWidth / 2, 58, { align: 'center' })
  ...
  if (concerns.length > 0) {
    ...
    doc.text(
      `This report analyzes ${concerns.length} PNP report${concerns.length !== 1 ? 's' : ''} across all districts`,
      margin + 10,
      yPos + 26
    )
  } else {
  ...
  const overviewText = `This report provides a comprehensive analysis of ${totalConcerns} PNP report${totalConcerns !== 1 ? 's' : ''} recorded across the Province of Bataan${dateRange !== 'all' ? ` during the ${periodText.toLowerCase()} period` : ''}. The data encompasses ${Object.keys(districtStats).length} legislative districts and provides detailed insights into the current status and geographical distribution of concerns.`
  ...
  districts.forEach((district, districtIndex) => {
    ...
    if (districtConcerns.length === 0) {
      ...
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(107, 114, 128)
      doc.text(`No PNP reports recorded`, pageWidth / 2, cardY + 46, { align: 'center' })
